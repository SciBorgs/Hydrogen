...
public class QuadSwerveSim {
   ...
   *
   * @param wheelBaseWidthM left-to-right distance between the wheel/ground contact patches, in
   *     meters
   * @param wheelBaseLengthM front-to-back distance between the wheel/ground contact patches, in
   *     meters
   ...
   * @param modules Set of swerve modules controlling the drivetrain. Order must be 0=FL, 1=FR,
   *     2=BL, 3=BR
   */
  ...
  public void update(double dtSeconds) {
    ...
    Vector2d velocity =
        new Vector2d(
            m_velPrev.x + (accel.x + m_accelPrev.x) / 2 * dtSeconds,
            m_velPrev.y + (accel.y + m_accelPrev.y) / 2 * dtSeconds);

    // Trapezoidal integration
    Translation2d posChange =
        new Translation2d(
            (velocity.x + m_velPrev.x) / 2 * dtSeconds, (velocity.y + m_velPrev.y) / 2 * dtSeconds);
