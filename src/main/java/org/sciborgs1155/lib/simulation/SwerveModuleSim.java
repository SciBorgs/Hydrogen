package org.sciborgs1155.lib.simulation;
...
public class SwerveModuleSim {
  ...
  private DCMotorSim m_azmthMotor;
   ...
   * @param azimuthGearRatio Motor rotations per one azimuth module rotation. Should be greater than
   *     zero
   * @param wheelGearRatio Motor rotations per one wheel rotation. Should be greater than zero
   * @param azimuthEncGearRatio Encoder rotations per one azimuth module rotation. Should be 1.0 if
   *     you have a good swerve module.
   * @param wheelEncGearRatio Encoder rotations per one wheel rotation.
   * @param treadStaticCoefFric Static coefficient of friction of the wheel tread and ground
   *     interface
   * @param treadKineticCoefFric Kinetic coefficient of friction of the wheel tread and ground
   *     interface
   * @param moduleNormalForce Downward force the robot and its mechanisms exert on this module.
   * @param azimuthEffectiveMOI Effective moment of inertia of the azimuth module. The Azimuth
   *     motion is modeled as a simple flywheel.
   */
  public SwerveModuleSim(
    ...
    this.m_azmthMotor = new DCMotorSim(azimuthMotor, azimuthGearRatio, azimuthEffectiveMOI);
    this.m_wheelMotor =
        new MotorGearboxWheelSim(
            wheelMotor, wheelGearRatio, wheelRadiusM, m_wheelGearboxLossFactor);
  ...
  /**
   * Returns the current physical rotation count of the azimuth encoder axle shaft in total
   * rotations since last reset - not wrapped. IE, if you spin your module exactly two and a half
   * times in the positive direction, this will return 2.5 (not 0.5).
   */
  public double getAzimuthEncoderPositionRev() {
    return m_azmthMotor.getAngularPositionRotations() * m_azimuthEncGearRatio;
  }

  /**
   * Returns the total number of physical rotations the wheel has done since the last reset - not
   * wrapped. IE, if your wheel rotated through 20.5 rotations, this function would return 20.5 (not
   * 0.5).
   */
  ...
  void update(double dtSeconds) {
    ...
    m_azmthMotor.setInputVoltage(m_azmthVoltage);
    m_azmthMotor.update(dtSeconds);

    // Assume idealized azimuth control - no "twist" force at contact
    //  patch from friction or robot motion.
    m_curAzmthAngle = Rotation2d.fromDegrees(m_azmthMotor.getAngularPositionRotations() * 360);
  }
  ...
  Vector2d getModRelTransVel(double dtSeconds) {
    double xvel =
        (m_curModulePose.getTranslation().getX() - m_prevModulePose.getTranslation().getX())
            / dtSeconds;
    double yvel =
        (m_curModulePose.getTranslation().getY() - m_prevModulePose.getTranslation().getY())
            / dtSeconds;
  ...
  ForceAtPose2d getCrossTreadFricForce(Force2d netForce, double dtSeconds) {
    ...
    boolean useKinFric =
        Math.abs(m_crossTreadForceMag) > m_treadStaticFricForce
    ...
    if (useKinFric) {
      ...
      m_crossTreadFricForceMag = -1.0 * Math.signum(m_crossTreadVelMag) * m_treadKineticFricForce;
    } else {
  ...
  ForceAtPose2d getWheelMotiveForce() {
    return new ForceAtPose2d(
        new Force2d(m_wheelMotor.getGroundForce_N(), m_curAzmthAngle), m_curModulePose);
  }
